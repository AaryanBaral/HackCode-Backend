...
namespace UserService.API.GlobalExceptionHandler
{
    public class GlobalException : IExceptionHandler
    {
        ...
        private static (int statusCode, string title) HandleSqlException(SqlException sqlEx)
        {
            ...
            {
                2627 => ((int)HttpStatusCode.BadRequest,
                    "Duplicate entry, unique constraint violation."), // Primary key violation
                ...
                4060 => ((int)HttpStatusCode.InternalServerError,
                    "Cannot open database requested by the login."), // Database connection issue
